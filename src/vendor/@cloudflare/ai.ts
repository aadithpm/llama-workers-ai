...
var AiImageToText = class {
  ...
  generateTensors(preProcessedInputs) {
    if (this.modelSettings.generateTensorsFunc) {
      ...
      return [
        new Tensor(
          'int32' /* Int32 */,
          [preProcessedInputs.max_tokens || 512],
          {
            shape: [1],
            name: 'max_tokens'
          }
  ...
  postProcessing(response) {
    if (this.modelSettings.postProcessingFunc) {
      this.postProcessedOutputs = {
        description: this.modelSettings.postProcessingFunc(response)
      };
    } else {
    ...
    if (this.modelSettings.generateTensorsFunc) {
      ...
      if (preProcessedInputs.image && preProcessedInputs.mask) {
        ...
        return [
          ...
          new Tensor('int32' /* Int32 */, [preProcessedInputs.num_steps], {
            shape: [1],
            name: 'num_steps'
          })
        ];
  ...
  generateTensors(preProcessedInputs) {
    if (this.modelSettings.generateTensorsFunc) {
      ...
      return [
        new Tensor(
          'int32' /* Int32 */,
          [preProcessedInputs.max_length || 1024],
          {
            // sequence length
            shape: [1],
            name: 'max_length'
...
var vLLMGPostProc = response => {
  let r = response['name'].value[0];
  return r;
};
var defaultvLLM = {
  type: 'vllm',
  ...
  postProcessingFunc: r => vLLMGPostProc(r),
  postProcessingFuncStream: r => vLLMGPostProc(r)
};
var defaultTGI = (promptTemplate, defaultContext, ignoreTokens) => {
  return {
    ...
    postProcessingFunc: r => tgiPostProc(r, ignoreTokens),
    postProcessingFuncStream: r => tgiPostProc(r, ignoreTokens)
  };
};
var modelMappings = {
  'text-classification': {
...
var modelSettings = {
  ...
  // vLLMs
  '@cf/thebloke/yarn-mistral-7b-64k-awq': defaultvLLM,
    ...
    for (var t in tasks) {
      if (modelMappings[tasks[t]].models.indexOf(model) !== -1) {
        ...
        if (this.options.apiGateway) {
          ...
          this.task.tensors = this.task.generateTensors(
            this.task.preProcessedInputs
          );
